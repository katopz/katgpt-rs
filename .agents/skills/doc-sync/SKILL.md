...
|---|---|---|---|
| 2026-09-06 | riir-clippy post-Batch-117 window (idle-loop unit; 851 v6 league live on the box — markdown-only unit chosen deliberately; rule IDs verified against `src/domains/kernel_opt/corpus/`) | 4 surfaces stale, all fixed: kernel_opt.md header "Batch 1-114: 518 rules" → 1-117/544 + **5 missing batch rows** (115/115-add/116/116-add/117 — the same per-batch-row gap the rust_perf ledger catch-up found at B112, now on the kernel side); AGENTS.md kernel_opt bold count 538 → 544 (its own ledger said →544 — the bold/parenthetical split is the trap); README domain table stale FOUR rows (clippy 45→49, rust_perf 50→101, kernel_opt 68→544, sec 6→12) + the corpus structure comment (46/68 → 101/544); 11_domains index (39/6/27-rule rows); rust_perf.md += B113's 2 name-field riders | clippy `d7fd954` |
