...
|---|---|---|---|
| 2026-09-11 (idle unit, fresh-context; three sibling compile lanes live — katgpt-rs plans 590-592, dapps/dao kat protocol, riir-ai Plan 583 tail — so grep/diff-only, committed content via `git grep HEAD` + diff-scoped greps; the dirty `hebbian_fact_edit/architecture.rs` audited from `cab3d0a71`'s committed blob) | Mode 2 drift audit — the post-09-10-evening fresh wave: mop_homeostasis arc (Plan 579 runtime `mop_runtime/*` + Plan 580 civ affect phase + Plan 581 swarm-forager promotion), deliberation saddle-trap flip leg (Plan 582), demo-feel-2 quest_combat (riir-ai 029d0abf6 + seal ee24360b), Issue 912 T3/T4 PoCs, Plan 583 fact-edit arc (engine + ndb KeyedFactSet/K2), Issue 915 civ gate hygiene + spatial_grid total-on-Default | **CLEAN — zero parallel systems, zero issues filed. The homeostasis arc is a model-consumer showcase; every named substrate consumption verified in-source.** | Diff-scoped families (sqrt/sigmoid/exp, spatial HashMap, belief/emotion struct defs, RNG/splitmix, last_known) over the 12 fresh commits' added lines only. **[1] mop_runtime (Plan 579) consumes katgpt_core wholesale**: `hmm_control::{HmmControlSolver,HmmSolution}`, `mop::MopConfig`, `occupancy`, `InducedCwmKernel`, `simd::fast_sigmoid` (imported — the riir-auth delegating-wrapper precedent, NOT a re-copy); both production consumers import `riir_engine::mop_runtime` (civ `map_tick/mop_homeostat.rs:48`, swarm `swarm/homeostat.rs:57`) — the model direction end-to-end. **[2] deliberation flip leg consumes `katgpt_core::saddle_escape::FlipDetector`** (:61 import, :612 field, :628 ctor) — the 977a9777 Copy-derive prep paying off; zero parallel flip machinery; the file's pre-existing sqrt sites untouched by the diff. **[3] alignment PoC (912 T4) consumes `katgpt_core::mag::transfer_score(TransferMetric::Wasserstein1d)` + `committed_field_blend`** — the gw_alignment metric half consumed, not re-implemented; the topological half DEFERRED per commit; in-source dec-trap doc (Plan 591 P0 #2). **[4] hebbian_fact_edit (Plan 583) consumes ndb + katgpt-core**: `freeze::MerkleFrozenEnvelope`, `construction_audit_sidecar`, `hebbian_bridge::{HebbianConstructedShard, KeyedFactSet, MarginalReport}`, `katgpt_core::HebbianSlot`; BRIDGE_DIM cited from the substrate, not re-defined. **[5] `drive_uniform` splitmix-STYLE arrival (swarm/homeostat.rs)** — murmur3-fmix + golden-ratio hybrid, documented ("the wander-tier's hashing discipline, no stored RNG state"), substrate-SIDE riir-games, reuses `super::math::CellKey` + `FxHashMap` (the existing spatial substrate, not a new grid): census note only — the 09-08 splitmix census counts substrate sites separately; consumer-side count unchanged at 1. **[6] Exempt classes confirmed**: `lcg()` in fact-edit tests is `#[cfg(test)]` (Knuth MMIX constants, not splitmix finalizer); `FearAccums` lives in the bench_905 test; seal-view's one screen-space sqrt is the view-plane exempt class (popup placement); the view consumes `DamagePopupRoster`/`ITEM_ID_MONSTER_LOOT`/`DAMAGE_POPUP_TICKS` FROM substrate quest_combat, not its own popup vocabulary; `r*rng.f32().sqrt()` = disc radial sampling, not distance; authority_sim/mod.rs:1463 + hero_routine.rs:5015 + world_build.rs:759 sqrts all pre-date the diffs (diff-scoped grep zero adds). **[7] spatial_grid fix = total-on-Default guards** (`cell_starts.is_empty() → None`) — a panic fix adding NO structure; the exact call-site lattice cfg documented in-file. Sibling safety: riir-ai/ndb/seal read-only; only this skill file staged in katgpt-rs. |
| 2026-09-10 (evening session, zero-contention unit — v9/851 hold live; all issue/plan lanes claimed: FSM sibling's fresh riir-ai 910 + seal 016/017, the Plan-023/024 kat-service arc mid-MOVE on the dapps worker, the paper sibling's untracked kat 543/590; fresh set since the morning row: the ProjectionAttestation producer→seam half that row's chain-module scope predated — ndb `attestation_seam.rs` 253 LOC + `project_cold_attested` (6f66c96/e0ee395/cebd1c7, repo clean) + riir-ai `tick_cold_attested` (895b99f24, audited from committed content via `git show` — tree sibling-dirty) + the chain sink half (c36b4829…6d3f527b, post-morning-audit) | Mode 2 drift audit — the attestation commit seam end-to-end | **CLEAN — zero parallel systems, zero issues filed.** | **[1] The seam IS the cited pattern, not a twin:** `AttestationCommitSink`/`AttestationSubmitReport`/`AttestationSubmitRefusal` mirror `engram_seam`'s shape only (trait + `()` default + object-safe + report enum) with disjoint vocabulary (table-identity vs attestation/refusal outcomes); storage-leaf hosting rationale restated in-header (chain → storage, engine → storage, never engine↔chain). **[2] Direction hashing single-sourced across THREE crates:** producer builds via `DirectionCommitment::canonical_axis` (ndb neuron_vessel.rs:161); `project_cold_attested` deliberately CONSUMES the caller-supplied commitment ("why the caller supplies the commitment" — the sidecar refuses to know axis-set policy, the right DRY direction); chain sink's evidence pre-check consumes the same crate's pub `commit_canonical_axes` (:220) instead of re-hashing. **[3] Sync-boundary holds:** `last_affects[k] = sigmoid(projected.scalar)` is the 5-scalar latent bridge; the 120-byte commitment is what the record carries; `axes` crosses the sink by BORROW for in-process evidence reconstruction only (documented in-seam) — the wire never sees the direction vector. **[4] `fn sigmoid` in runtime.rs is the delegating-wrapper false-alarm class** (→ `katgpt_core::simd::fast_sigmoid`, pre-existing, body-read verified — the riir-auth session.rs precedent). **[5] Replay discipline:** runtime-owned `attestation_nonce` post-incremented per submission via disjoint-field destructure; chain refuses by the full-record BLAKE3 (single-sourced per the morning row). **[6] Both crates independently converged on the one-persistent-Store shape** (engine `tick_cold` root-cause fix + chain sink `6d3f527b` perf fix) — the store-bound `OnceLock<CompiledVessel>` constraint answered the same way twice, consistent family solution; the engine fix is regression-pinned in attested_tests. **[7] Refusal vocabulary distinguishes honest (Replay) / caller-bug (NoDirectionEvidence, EvidenceUnavailable) / fraud-observation (VerifyFailed) classes in-doc; slash POLICY stays chain-side (`AttestationSlashReason`, a different plane). Sibling safety: ndb/chain read-only clean trees; riir-ai never touched (committed content only); this row staged alone in katgpt-rs. |
| 2026-09-10 (third later session, zero-contention unit — v9/851 hold live, grep/read-only; fresh set since the day's two earlier rows: riir-ai `da70607e0` qsg_crowd (Plan 577 / Issue 907, LANDED 09-10, 457 LOC, default-off `qsg_crowd`) — the only game-plane System code landed past the "second later session" row's quest_gfn set; Batch 142 (riir-clippy corpus data) cannot introduce parallel systems — skipped; `deliberation.rs` dirty = sibling WIP, excluded as live) | Mode 2 drift audit — the QSG rumor-consensus consumer (`riir-games/src/swarm/qsg_crowd.rs`) | **CLEAN — zero parallel systems, zero issues filed. The issue's own "substrate-consuming" Type claim VERIFIED in-source.** | **[1] Kernel consumption TOTAL** — all math delegates to `katgpt_core::qsg_gossip` (11 imported pieces: init/sample/draw/blend/mean/U/V/uncertainty/pair + types; zero re-implemented gossip or simplex math). **[2] The one inline site (L286–293, the exponential tilt `p̃ ∝ p·e^h`) is the kernel's own DOCUMENTED composition recipe** (qsg_gossip module doc: "Compose it from the exported pieces: tilt a copy of the speaker row, qsg_draw_categorical from the tilted copy, qsg_blend_listener_into — see the scaling-law test for the exact recipe") — the kernel is neutral-by-construction, the recipe lives consumer-side BY DESIGN; not duplication. **[3] Family-level DRY holds** — `swarm/mod.rs:155-158` pins the crowd-family axis split ("signed_coupling owns the T axis (social_pressure), qsg owns the m·α axes, mean_field the reducers — do not blur, compose"); `social_pressure.rs` grep-verified to carry NO belief-simplex/blend surface (its "appeal to authority" is a classification label) — no overlap with the gossip blend. **[4] Two-brain + sync-boundary compliant by construction** — grep-verified: zero serde/SyncBlock/GossipDelta/ZoneDelta/KG-emission surface in the file; beliefs are think-brain local latent; only the U/V scalars surface (the 5-affect-scalars/flock-centroid precedent); KG emission explicitly stays with the zone host's existing `GossipedWith` path. **[5] Bridge pattern holds** — `clamp_alpha` (persona scalar → kernel α ∈ (SIMPLEX_EPS, 1], fail-to-1.0 on non-finite, zero-alloc) is the sanctioned bridge shape; `budget_bandwidth_m` composes `npc_mind_reading`'s `k_for_coverage` rather than re-deriving a budget. **[6] RNG/replay discipline** — every draw rides the caller's `UniformStream`; replayable trajectory; `tilt_toward` panics on out-of-range (authority-safe). Feature-gated default-off per the no-default-consumer rule (Issue 907 records the promotion gate). Sibling safety: riir-ai tree read-only this session; only katgpt-rs skill-file row staged. |
| 2026-09-10 (second later session, zero-contention unit — the v9/plan391 hold STILL live (pair1_gate at 99.1% CPU, v9 parked, the test --all-features sibling idling at 0.0%), so grep/read-only; fresh set: the sibling quest lane's quest_gfn MDP landing `10045f09e` (Plan 368 Phase 1, quest_grammar, 610 LOC) — the LANE IS STILL ACTIVE (dirty `quest_tpr.rs` + `issue847_ood_bench.rs`), so the audit ran against COMMITTED content via `git show 10045f09e:…`, never the working tree; riir-games dirty files (crowd_mcgs/two_brain, game_sync/trade, motivation/needs) are uncommitted sibling WIP — excluded as live | Mode 2 drift audit — the quest_gfn constructive MDP + closed-form energies (the landing's own module header claims a substrate-first gate, dated same-day) | **CLEAN — zero parallel systems, zero issues filed. The lane's own Mode 1 record VERIFIED ACCURATE in-source: all four claimed consumptions are real and load-bearing.** | **[1] The four header claims verified:** `ConstraintPruner` is IMPLEMENTED, not just imported (`impl ConstraintPruner for QuestSlotPruner`, :186 — the mask contract is the same trait the drafter rollout uses, the designed extension point); `GrammarValidator` genuinely consumed (field :250, construction :261, `underspecification_score` :315, `validate` :495 — "never forked" holds); `emit_text` consumed for canonical emission (:118); the agreement mask COMPOSES the substrate (`filler_class_full` :198-215, `filler_number` :208/:299/:438, `role_legal` :416 test parity) — no re-implementation. **[2] Zero math/RNG families:** sigmoid/exp/rand/Rng/splitmix/sqrt → 0 hits (deterministic closed-form arithmetic by construction). **[3] Zero sync/spatial/belief surface:** SyncBlock/Serialize/MapPos/Belief/spatial/HashMap → 0 hits — the MDP is a pure prefix tree over vocabulary indices, nothing to sync (the raw/latent rule trivially holds). **[4] No pre-existing MDP/GFlowNet substrate paralleled:** the workspace census (gflownet/trajectory_balance/backward_policy) hits only katgpt-speculative's `FlowPruner` (a screening pruner: lambda/stop-probs/flow-bonus for token drafting) + `dd_tree` (width-selection config) — a DIFFERENT plane with different contracts; the quest domain had no MDP substrate to consume, and the domain-specific types are the correct new-substrate shape. **[5] Energy family:** katgpt-core's "energy" hits (cp_hopfield, tucker, dirichlet, linoss) are module-internal mathematical objects with no consume-able general contract; the quest energy's inputs come from `GrammarValidator` — the closed-form is domain arithmetic by necessity, delegating its INPUTS not its formula. **[6] En-route:** the module header records `log P_B ≡ 0` (fixed slot order ⇒ tree ⇒ point-mass backward policy) "so the trainer does not re-derive it" — exactly the doc-as-spec discipline; the frozen-likelihood seam is a documented Phase-2 extension point, not a stub presenting as functional. Sibling safety: this row staged alone in katgpt-rs; the audited repo untouched. |
| 2026-09-10 (later session, zero-contention unit — the v9/plan391 hold STILL live (pair1_gate at 99% CPU, v9 autofire waiting, two sibling builds active: chain clippy passes + a cargo test --all-features), so grep/read-only; the fresh set is the arc the morning row EXCLUDED as live: the seal-remake Plan 005/006/007 + Plan 575/576 wave, now LANDED and quiet (seal HEAD `b8cb22b4` == origin, guard ALL LAYERS GREEN; ai side `3bab57572`/`2087f8a12`/`875c8fdeb`/`280010150`/`668f5ae44`/`1099c9e65`/`24aae9bc0`) — 13 game-plane files audited; the riir-gpu files in the same range are Issue-906 test-arming + the 0e80cf26a lint sweep (mechanical, cannot introduce parallel systems — skipped) | Mode 2 drift audit — the post-arc fresh wave: attack-reasoning ruliology (Elo book / combo FSM / INT ladder / heal-retarget), hero attack-hold FSM, hero_intelligence Arc seam, quest board parameterization, HeroStats peer-boot seam + the seal-side stat ferry | **CLEAN — zero parallel systems, zero issues filed. Six substrate consumptions verified in-source; two documented twins (copy-gate class, below consolidation thresholds).** | **[1] AttackEloBook (attack_reasoning.rs:728 LOC): SUBSTRATE CONSUMER** — `use katgpt_core::rating` (:39); expected via `rating::expected_f32(cell.rating, OPPONENT_ELO, 400.0)` (:256,:267), K = `rating::STANDARD_K` (:215); the commit's "hint_regret deliberately not pulled" is a documented dep decision, and ZERO inline `1/(1+10^x)` hits — the 09-04 Elo census family INTACT in fresh code. **[2] element counter-pick consumes `games_shared::game_traits::element::damage_multiplier` (:40)** — no new element math. **[3] UCB1 bonus (:300-316) is a pure fn of visit counts, "deliberately not RNG" in-source** — authority-replay determinism stated; its `sqrt(ln/visits)` is exploration math, not spatial (false-positive class). **[4] combat_tick (combat.rs:109) DELEGATES to combat_tick_with_interval** — one state machine, the ladder scales cadence only, zero FSM drift; the legacy path is the parameterized path at `attack_interval_ticks(dex)`. **[5] quest_center_manager_with_kinds (quest_center.rs:110) is the DRY direction** — the parameterized general form; the env-toggle constructor composes it as the `[Tame]` special case ("One filter home"), doc cites the factory precedent. **[6] HeroStats (quest_combat/hero_stats.rs): feature-neutral placement ON PURPOSE** — module doc explains the gate-union decoupling ("a type must not bind two gated surfaces into one combo", the 280010150 fix); domain split stated in-source ("hp is a raw physical store component, weapon/int are think-brain Arcs"); the scenelab `HeroPreset` twin (runner/model.rs:117) is DOCUMENTED with the dep-direction reason (riir-games-mmorpg cannot name the downstream lab) — 3-Option-field copy with in-source citation = sanctioned copy-gate shape, NOT drift; the seal ferry (747c70a5) consumes the seam ("ONE authoring-stat vocabulary across solo + duo") with a round-trip probe. **[7] hero_intelligence seam (3bab57572) follows the hero_weapon Arc precedent** — Arc<AtomicU8>, default-identical to the previous hardcoded read, both swing paths read it. **False positives documented:** hero_routine.rs:4925 `mean + (delta²+c²).sqrt() − c` is the 2×2 PSD max-eigenvalue CLOSED-FORM TEST ORACLE (spectral-fusion sandwich test), not a distance; quest_combat/mod.rs:1084 is a no-sqrt hot-path COMMENT (the correct idiom); `MonsterSwing`/`EloCell` are domain vocabulary; zero hashmap-spatial, zero fog/last_known_pos, zero inline sigmoid in the fresh set. Sibling safety: katgpt-rs gained sibling dirties this session (doc-sync SKILL.md + 2 crates files) — this row touches ONLY the skill file, staged alone. |
| 2026-09-10 (zero-contention unit — v9 measurement hold live (851/876/886/887: any cargo resets the 600s quiet streak), so grep/read-only; fresh set since the 09-09 audits: chain attestation-commit module `b88c0261` + deployer faucet sidecar `a66e911`; the seal-remake Plan 006/007/058 arc (ai/sdk/mmorpg/seal, HEAD `400c895` 13 min old at audit time) EXCLUDED as a live sibling lane; katgpt-rs/ndb deltas are test-arming/docs only — cannot introduce parallel systems) | Mode 2 drift audit — the unaudited consumer-plane landings | **1 finding FILED (chain Issue 139, detection-only); faucet surfaces clean; zero parallel systems.** | **[1] chain `neuron_vessel_attestation_commit.rs`: the 15-line `TxDelta` composition (`:343-358`) re-implements `ChainClient::build_tx`'s declared-ONE-constructor field order, documented as a mirror at three doc sites — but the mirror is DOC-ONLY: the module's 7 tests pin self-consistency (checksum, alpha-key re-derivation, preflight acceptance, byte-determinism), none pins equality with `build_tx`'s own output. Copy-gate discipline requires a divergence-failing test for a justified copy; filed as chain Issue 139 (d773915c) with two fix options (equality pin vs `pub(crate)` reuse — `build_tx` reads only `self.player_uuid`+`self.network`, so a `build_tx_for(identity, network, …)` refactor serves both callers). Not a parallel system: the module consumes the substrate types; the copied thing is the composition order. **[2] papaya holding** — the replay ledger is `papaya::HashMap` (`committed.pin()`), per the lock-free rule. **[3] deployer faucet (`a66e911`): yaml + 2 docs only** — zero crypto/math/RNG surface to audit; the fail-closed `CHAIN_FAUCET_KEY` gate is config, and the health probe rides the existing `verify` probe family. **[4] replay-guard set single-sourced** — `attestation_record_commitment` (BLAKE3 over the full 120-byte record) is defined once, consumed by commit + adjudicate + `is_committed`; the record's own 88-byte-prefix checksum is a different axis, correctly not conflated. |
| 2026-09-09 (zero-contention unit, 52nd-boundary window; fresh set since the earlier-09-09 audit: ndb CLI crate `7dda44e` + Plan 328 landing `25ee879` (repo now clean), auth `account_key` substrate `c2274ca`, dapps burn-ack v3 contribution-fact `9df093b`+`7ed0168`, clippy client-half `587132b`+`a200721` committed parts; sibling IN-FLIGHT lanes excluded as uncommitted: riir-clippy kat_account migration WIP (12 dirty incl. score_bench/ood.rs), riir-ai 80 dirty — its 16 un-pulled origin commits diff-scanned instead) | Mode 2 drift audit — crypto-plane + account-id-derivation families over the fresh set + wire-pin and game-vocab checks on the new CLI | **CLEAN — zero parallel systems, zero issues filed. Three verifications worth recording.** | **[1] ndb CLI seal/unseal vs local_kv/crypto.rs = JUSTIFIED PLANE SPLIT, not drift:** the store crypto is `pub(crate)` (importable by no external crate — hard technical bar), the key domains differ (WAL passphrase-derived master key + `derive_nonce(key)` vs owner-seed keyed XOF + `nonce = blake3(logical_row)`), and `row.rs:21-31` documents the tradeoff citation (SIV-like nonce-reuse safety) + the client-side rationale (syncing servers hold only ciphertext — `use riir_auth::AccountKey` at row.rs:31 and `use riir_neuron_db::local_kv::LocalKvStore` at :37 are the substrate consumption, verified). The 5-instance `BLAKE3(pubkey)[..16]` copy-gate convention now has a sibling family — `BLAKE3(pubkey).to_hex()` (full 64-hex, documented bit-compatible with kat_account + the web-wallet .keys match) — and the 2nd family member (kat_account) is being consolidated onto the substrate by the in-flight migration agent, the DRY direction working. **[2] auth `account_key`: clean substrate** — `ssh_key` crate throughout (zero hand-rolled Ed25519), `account_id_for` = `blake3::hash(pubkey).to_hex()` pure fn, `seed()` doc names neuron-db-cli as the sealing consumer (possession-of-key = authority, the raw-domain rule), zero RNG in the CLI (deterministic sealing by design). NDBS/1 wire: one canonical request-builder fn (`sync.rs:480`), full layout in the module header — no mirror to drift. **[3] dapps burn-ack v3 + clippy client half: single-source derivation holds** — the 32-byte commitment is computed server-side only (dapps mining.rs); the clippy client treats `first_commitment_hex` as OPAQUE provenance (parse + carry, never re-derive — `kat_burn_client.rs:211` comment is explicit); no golden-pin obligation arises because the client computes nothing to pin. Fresh-wave direction check: mmorpg `7b2954a` (src/scenarios/ dissolves into substrate) is the extraction direction these audits exist to force; riir-ai `0e80cf26a` is a mechanical lint sweep with justified scoped allows — no parallel systems. Healer-corpus-gap pointer for the post-mining queue recorded in `0e80cf26a`'s commit message (needless_pass_by_ref_mut, warn-by-default 1.98, NOT in clippy_lints corpus) — left there because the batch lane owns the snapshot file this window. |
| 2026-09-09 (zero-contention unit after the boundary 46th run; fresh set since the 09-08 audit: dapps burn plane `src/kat/burn.rs` 700 + `burn_contract.rs` 353 + routes/crank (a2648c6, postdating that audit's dapps slice), chain AOI Phase 3a `aoi_verify.rs` 485 + `occlusion.rs` +300 (71bbfcfd/c2b595ff — the 058 agent's landed slice, audited read-only), dao pins test 190 (dd7d75a), clippy `kat_burn_client.rs` (f3a7cf2); seal f6460477 is Cargo.toml-only) | Mode 2 drift audit — distance/sqrt, float-money, RNG/splitmix, game-vocab, golden-pin, and occlusion-mirror families over the fresh set | **CLEAN — zero parallel systems, zero issues filed. One verification arc worth recording.** | **[1] chain AOI occlusion mirror = SANCTIONED, verified end-to-end:** `aoi/occlusion.rs` module header carries the full vocabulary-mapping table (`vision::Occlusion` ↔ `AoiOcclusion`, `NeverOccluded` ↔ `NeverOccluded`, `HeightfieldOcclusion` ↔ `HeightfieldOcclusion`) + a "Verifier symmetry (D6 — load-bearing)" section — Plan 057 D6's promised cross-reference EXISTS; the algorithm divergence (substrate stride/eye_height march vs chain half-cell/clearance/4096-cap nearest-cell march) is the TWO-BRAIN model, substrate's own doc disclaims anti-cheat use, Plan 058's substrate-first record pins the info-brain/think-brain split + the no-back-edge dep direction, and producer/verifier symmetry is on the CHAIN type (both sides arm chain `AoiOcclusion`) — no cross-mirror asymmetry risk. **[2] dapps burn plane clean:** money paths pure integer (i128/u64/MicroUnits — `as f32` hits are the front-page SVG chart polyline geometry only); zero RNG (splitmix consumer census stays at 1/4 threshold); zero game vocab; ledger ops consume `LocalKvStore` substrate. **[3] burn-message golden-pin chain INTACT across all three repos:** dapps `burn.rs:53` mirror note + `tests/kat_burn.rs:44` golden bytes ↔ clippy `kat_burn_client.rs:532` same-bytes pin ↔ e2e fixtures sign via the canonical fn — the 023-design tripwire discipline shipped and holding. **[4] dao pins test is a golden FIXTURE (hard-coded expected bytes modeling the shipped world incl. the KAT_FAST_SETTLE=off epoch), not a re-derivation — the correct pin shape.** |
| 2026-09-08 (zero-contention unit after the boundary-guard 42nd run; post-09-05 landed wave: 173 fresh consumer `.rs` files across 7 repos — mmorpg ~95 (Issue-092 fallout + viewbridge-node fresh), chain 46 (pay_request + gm_keys_cli + nft plane), dapps 15 (kat-service + item_nft + pulse), ndb 12 (613 NaN-sink + 495 leakage bench), viewbridge 4 (entities/net/clock/protocol), dao 1, deployer 1; sibling IN-FLIGHT lanes excluded as uncommitted: mmorpg Issue 100, seal wire-client, chain gm-keys dispatch (its ahead=1 commit), train 614) | Mode 2 drift audit — 7 grep families (distance/sqrt, inline sigmoid/exp, hashmap-spatial, belief/perception defs, emotion defs, game-rule defs, last_known_pos) + repo-specific spot checks | **CLEAN — zero parallel systems, zero issues filed. Two note-level censuses.** | **[1] splitmix64 finalizer census (NOTE-LEVEL, consolidation opportunity — not filed):** ~20 private copies across katgpt-core (each module ships its own `SplitMix64`/`next_u64` — cgsp, conformal, cp_hopfield, content_store, effective_degree, factorized_action, group_invariance, hebbian, kinematics/fixture, manifold_bandit, mi, product_key_memory, pruners, similarity_inference `fn mix` (slight variant), tether) + 5 riir-games substrate sites (math.rs `wander_angle` the canonical; luck.rs/spawn_zone.rs/stochastic.rs/coverage_curiosity.rs cross-document it) + 2 civ harness RNGs (G8Rng/HlaRng — harness class) + ONE consumer arrival: mmorpg `crates/riir-viewbridge-node/src/lib.rs view_id` (splitmix over UUID hi^lo for view-stable render ids — documented rationale, ~3 muls/entity/frame perf justification, collision pin test `view_id_does_not_collapse_low_half_only_synthetic_ids`). Classification: constant-recipe re-derivation with ZERO behavioral divergence risk (all bit-identical to Vigna's reference), every copy documented or trivially standard; the BLAKE3(pubkey)[..16] 5-instance copy-gate precedent applies — a consolidation (`katgpt_core::rng` export or `games_shared` view-id mix) becomes worthwhile at the 4th CONSUMER-side arrival, currently 1. Not a ThreatField class (no domain logic). **[2] ndb 611 sigmoid delegation HOLDING:** the fresh `shard_compactor.rs:957 fn sigmoid` is the ±50-clamp delegating wrapper the 611 fix itself created (doc cites the issue) — not a re-copy. **[3] chain 46 fresh files: no game rules** — vocab hits are `pay_request` substrings + the sanctioned gm_keys/gm_sock GM control-plane CLI (the riir-mcp-client admin surface, `src/programs/` untouched); the program_rate_gate's 5 detectors own this repo per-push. **[4] dapps: direction-gate discipline holding** — `types.rs` game-vocab hits are its own rule-stating doc-comments; kat-service hits are `Request` substrings. **[5] QuestCombatNode (mmorpg viewbridge-node:133): model adapter** — documented orphan-rules newtype over `HeadlessHarness` implementing the NodeSim FFI port; view-layer id derivation, not game logic. F1's single hit (`game/tests.rs:874`) is a test fn — exempt class. |
| 2026-09-05 (later session, zero-contention unit — all issue/plan lanes claimed by live siblings: katgpt-rs vrde/513/725 checkout diverged with dirty WIP, riir-ai 092-B + 851 + quest-combat substrate, game-sdk scenelab 029, riir-clippy/riir-deployer KAT wet-run 061, mmorpg 092 extraction; heavy builds skipped so 851's autofire is not further delayed) | **Mode 2 drift audit — the post-09-04 fresh wave**: riir-chain NFT ledger plane (Issues 122/123/124/125: `asset_lifecycle/nft_transfer.rs` NEW 674 LOC + `nft_ledger` + token_registry dest-guard + the NftProgram retirement fallout across programs/), riir-dapps `item_nft::transfer_ownership_sidecar` + payreq claim/treasury routes + kat-service wet-run contract, riir-deployer `ceremony.rs` T2.0 (postdates the 08-29 deployer audit), seal-remake `authority_sim_seam.rs` (092 adoption); ndb + dao fresh files are clippy-1.98 mechanical batches only (cannot introduce parallel systems — skipped); riir-auth quiet | **CLEAN — zero parallel systems, zero issues filed. All four repos verified clean (0 dirty) before auditing; model consumers dominate.** | The 6 grep families (distance/sqrt, sigmoid/exp, hashmap-spatial, belief/perception defs, emotion defs, game-rule defs) over the 16 fresh files + body reads of every candidate. **[1] seal `authority_sim_seam.rs`: MODEL CONSUMER** — imports `AuthorityBootConfig`/`AuthorityWorldHandles`/`PredicateLayers` from `riir_games_mmorpg::authority_sim`, composes the predicate through `riir_games_cluster::make_validator_predicate`, consumes `snapshot_vocab` kind consts + `quest_combat::combat_state_byte`; the swarm-state ladder (stamina/thinking/moving) is composed inline from substrate CONSTS because the substrate ships no classifier fn — identical to the reference consumer's projector.rs:293 (2 copies of a 6-line consumer composition, below the promote-on-5th convention; NOTE-LEVEL). **[2] chain `nft_transfer.rs`: clean** — injected-verifier trait shape (Plan 319 `GmSignatureVerifier` pattern), `nft_transfer_signing_message` is the ONE canonical fn (:262) and dapps IMPORTS it (item_nft.rs:36,191 — the model compose, zero message-layout re-implementation); kat-service worker has NO nft surface (its `signing_message` hit is the KAT plane's own contract test). **[3] `BLAKE3(pubkey)[..16]` family census: 5 documented, individually-gated instances** (riir-wallet-signer `identity.rs:24` pub fn; chaind `chainspec.rs:336` private — standalone-parse rationale; dapps `account_for_pubkey` kept local for feature independence + golden-pinned equal to `submitter_for` in tests/kat_payreq.rs; deployer ceremony.rs:477 explicitly a CROSS-CHECK, loader-authoritative per the committed contract row + real-parser probe 09-04; clippy `mining::submitter_for`) — each carries its own rationale/pin, the copy-gate discipline working, not drift; a 6th arrival should consider consolidating onto `riir_wallet_signer::player_id_hash`. **[4] dapps `ItemNftRecord` vs chain `NftRecord`: different planes by design** (catalog binding vs ledger row; in-header: "the chain never reads any field here"; game-side persistence routed to Warm tier per Plan 002). **[5] deployer `ceremony.rs`: keys via `ed25519_dalek::SigningKey::from_bytes`** (no hand-rolled math; ssh wire form deliberately NOT reconstructed, documented); the 08-29 audit row's zero-sibling-dep + RAW-domain findings unchanged. **[6] ceremony's `BLAKE3` seeds are domain-separated per role** (the existing convention). False positives documented: chain `cpi.rs:619 checked_sqrt` (AMM LP formula — the standing statistical/financial class, no spatial operand). Sibling safety: zero state change in the shared katgpt-rs checkout — this row authored in a /tmp worktree off origin/develop; the local checkout's diverged develop + dirty vrde WIP untouched. |
| 2026-09-05 | Mode 1 gate on a FILED ISSUE (pre-code): game-sdk Proposal 029/030 external review filed riir-ai Issue 870 proposing new substrate (`StatMods { at, hp }` + additive `stat_total = base + Σ mods + Σ equipped` in riir-games-shared); the acceptance gate ran before any implementation | **FALSE POSITIVE caught — the substrate EXISTS. Issue 870 resolved SUBSTRATE-EXISTS (riir-ai `ebe8fc224`, file removed); game-sdk 030 re-pointed (9 touchpoints, `b8c4a50`). Zero code landed — the ThreatField-class parallel system prevented at filing time.** | **A NEW vocabulary-translation failure class, the issue's own grep refuted twice:** (1) operator-name — `stat_total\|StatMods\|[Bb]uff` misses `riir-games-shared::stat_modifier` (Plan 411 GOAT, Issue 502; `StatModifier` logit deltas + `ModSourceKind::{Equipment,Buff,Debuff,Terrain,Party}`, sigmoid-clamped `resolve` `[0,2·base]`, `delta_for_factor`, CAP-8 zero-alloc `StatModifierStack`); (2) **crate-name — the grep targeted `mmorpg` (riir-mmorpg-examples) and missed `riir-games-mmorpg`, whose `pet_alarm.rs` guard rail the issue ITSELF cited as "the only buff-shaped code to generalize" — it is ALREADY a substrate consumer (`PetAlarmState.guard: StatModifierStack`; `delta_for_factor`'s doc cites Plan 022 Phase G). Semantic proof the additive proposal was wrong, not just redundant: `resolve` clamps `[0, 2·base]` — an unbounded additive card total would display values combat cannot produce (+12 flat buff on base 11 exceeds the 2× ceiling). Lesson for every future census: translate crate names too (the `X` vs `riir-games-X` prefix family), not just type/fn names. |
| 2026-09-04 (05:0x, zero-contention unit — `digit_ood_eval` at 223% CPU + the 851 MLX ranking `--wait-quiet 600` armed + sibling Batch-104 GOAT in `/tmp/riir-clippy-b104` → grep/parse-only per the Plan 337 discipline) | **Mode 2 drift audit — the post-08-29 fresh wave**: 092 S1–S3 extraction (~10K LOC consumer→substrate moves in `riir-games-mmorpg`), mmorpg lineage surface (Issue 094), dapps KAT crank/alarm legs, dao yield-ceiling mirror, seal-remake viewer/node fresh code, chain pay_request + asset-lifecycle/forensic, and the VERIFICATION half of the three copy-class campaigns (853/861 percentile, 867 Elo, 087 triage) | **1 finding FILED (ndb Issue 611, detection-only); the three copy-class campaigns verified TERMINAL/COMPLETE; zero parallel systems. The discipline held through the largest consumer→substrate move in workspace history.** | Definition-level greps (`struct\|enum\|trait\|fn` at line start) over the derived 16-repo set, `obsolete/` excluded, plus the three copy-class censuses and body-level reads of every candidate. **[1] nearest_rank/percentile census: COMPLETE, zero unaccounted copies** — canonical `katgpt_core::stats::nearest_rank` (861 promotion); documented specializations hold (games-shared stats.rs leaf-clean note, riir-e2e `7ac2315` 2nd specialization); `speculative/types.rs::nearest_rank_p99` is a different fn (rank-INDEX computer, exact-int `div_ceil`, own doc); `tpr/als.rs:551` + mmorpg `ccu_loadgen.rs:654` + civ `prod_latency_bench.rs:391` are INTERPOLATED percentiles (round/lerp on the (n−1) scale — a different statistic, auditor-SAFE class; converting als would rewrite BLAKE3-committed artifact bytes — correctly left alone). Auditor cross-check: 0 DEGENERATE / 0 TRUNC-VAR both repos. **[2] Elo census: COMPLETE** — `katgpt-pruners/arena/types.rs` expected/update DELEGATE to `katgpt_core::rating` (Issue 686 note in-source; counted in 867-T2.1's five); dao consumes `rating::*` constants directly (declared allowlist dep). **[3] triage/Regime: HOLDING** — `pet_teaching_677.rs::triage` is the τ-binding adapter over the SDK facade (087 fix INTACT through the S3 re-host); substrate `pet_teaching::frontier_regime_of` consumes `katgpt_core::hint_regret::triage` DIRECTLY — correct for substrate (the facade route is unavailable in riir-ai; a dep on the SDK would be a back-edge; the in-source comment documents the route history). Other `Regime` enums (mean_field, kinematics, data_probe, four_regime_router, jlens_poc, RegimeBehavior) are domain-vocabulary collisions, not copies. **[4] THE FINDING — riir-neuron-db carries 4 PRODUCTION scalar sigmoid copies while katgpt-core is an UNCONDITIONAL dep there**: `neighbor_heal.rs:100` (pub, zero external consumers — grep-verified), `shard_compactor.rs:952`, `local_kv/views.rs:76` (3 lines below a `katgpt_core::simd::simd_dot_f32` call), `interpolation_geometry:427` (audit-grade, self-documented mirror of shard_compactor); the crate's own `precision_drift.md` documents ULP-tolerance for exactly these paths, and `zone_geometry`/`dec_arena` production ALREADY consume the substrate `simd_sigmoid_inplace` (their local scalars are `#[cfg(test)]` parity refs — the delegation pattern exists in-repo). Filed as **ndb Issue 611** (`ffdc38a`, detection-only, highwater 610→611) with the freeze-commitment version-boundary caveat. **RESOLVED same day (ndb `6ae5435`): all 4 copies delegate — neighbor_heal's pub fn deleted (3 call sites + the lib.rs re-export dropped with it; zero external consumers re-verified), shard_compactor + interpolation_geometry keep their ±50 clamp and delegate the exp, local_kv::views::bridge_project delegates beside its existing simd_dot_f32 call; deliberate keeps stand (substrate_geometry_audits CommittedFieldBlend mirror, sigmoid_expected oracle, 2 cfg(test) parity refs). Count-identical validation 420/0/4 default + 627/0/8 union, clippy -D clean both states; version-boundary note carried in the commit message. En-route find: Cargo.lock is gitignored in ndb and the local lock had drifted to tinyvec 1.13.0, which fails to compile on this box's default rustc 1.93 (vec! macro-resolution through `use alloc::vec::{self, Vec}`) — both local locks downgraded to 1.8.1, zero repo change.** Deliberate keep: `substrate_geometry_audits.rs:207` clamps ±50 because it MIRRORS CommittedFieldBlend's gate formula (a different function, not `katgpt_core::sigmoid`). **[5] riir-chain sigmoid census: NOTE-LEVEL, no issue** — `forensic/recover.rs` f64 = REQUIRED (`chain_forensic = ["dep:blake3"]` only); `congestion.rs` inline = REQUIRED (`chain_congestion_control` pulls no katgpt-core); engine-bridge `lora_posterior.rs` = REQUIRED BY DESIGN (the crate exists to keep katgpt out of the chain Docker context); `consensus/curator_bridge.rs` + `curator_reward.rs` sit under `chain_curator` (dep:katgpt-core available) BUT all three consuming fns have ZERO production callers and `compute_accuracy_score` is test-only — production-dead surface carrying latent duplication; delegate-or-prune when consumers materialize. Also: `katgpt_core::sigmoid` is Cephes (~1 ULP), NOT bit-identical to the chain copies' libm-exp forms — delegation there is a semantic change even where the dep exists. `block_backoff.rs` delegates correctly (chain_guard exists to arm exactly that). **[6] 092 moved substrate code: CLEAN** — all S1–S3 modules import substrate paths (`GenericSpatialBelief`/`MapPos3D`/`social`/`swarm`/`game_sync`) + crate-internal siblings; no re-implemented substrate. **[7] lineage.rs (094): model consumer** — composes ndb `MerkleFrozenEnvelope`/`HEADER_LEN`/`GENESIS_PARENT_COMMITMENT`; blake3 only for per-envelope integrity. **False positives documented:** riir-viz `QuestLayerVisibility`/`LayerVisibility` (render-layer masks, view layer); predation `Fov*SwarmEmotionSystem` (documented model-consumer wrappers); `crowd_demo` sigmoid/`FearColorConfig` (view color mapping); `NullAffectField` (canonical substrate default); riir-auth `session.rs::sigmoid` (a DELEGATING wrapper over `katgpt_core::sigmoid` — def-grep false alarm, body read required); dapps kat/ has zero latent/sigmoid gating of money decisions (domain rule holds); seal-remake + viewbridge fresh code clean (viewbridge's 2 fresh files are xtask/FFI = boundary-guard territory). Method note: `$(var)` is command-substitution syntax and EXECUTES a command named by the variable — the repo-set helper must be a shell FUNCTION `consumers() { …; }` called as `$(consumers)`, or the grep silently falls back to scanning the whole tree (this run's first attempt did, and `obsolete/` hits had to be filtered; the corrected form inlines the set). Sibling safety: ndb's 46 dirty src files = live sibling WIP — the finding touched only `.issues/` (+highwater); riir-ai's bench/rematch dirties untouched. | **Mode 2 drift audit — the 11 repos the fixed Step 2 can now reach** (riir-armageddon, riir-dapps, riir-dao, riir-deployer, riir-viewbridge, riir-auth, riir-burner, katgpt-web, riir-clippy, seal-game-editor; riir-unity has 0 `.rs`). ~700 `.rs` files. Ran immediately after the instrument fix in the row below — a blind gate's clean history is not evidence. | **CLEAN — zero parallel systems, zero issues filed.** Two definitions looked like candidates and both survive inspection: `riir-auth/src/kg.rs:66 AuthKgTriple` CONSUMES `riir_neuron_db::vibe::KgTripleTemplate` and converts into it (a domain enum over the substrate, with the sync-boundary rule stated in its header — the model citation, not a violation); `seal-game-editor/…/model_3d_view.rs:381 SceneSpatialIndex` is a private AABB BVH over mesh triangles for viewport ray-picking — a rendering structure, not the game-world `GridSpatialIndex` grid. `riir-armageddon` consumes `GenericSpatialBelief` correctly (3 files). | Two definition-level grep families over the set: the Step 1 substrate vocabulary (belief / emotion / affect / spatial-hash / swarm / fog / sync-delta / KgTriple / NeuronShard / MapPos / ForceVector) and the game-rule + cognition family (Goal / Drive / Motivation / Reputation / Karma / Quest / Crafting / Bounty / Threat / Perception / Cognition / Memory / Consolidat). Definitions only — `struct|enum|trait|fn` at line start — because CONSUMPTION is the desired outcome and only a *definition* can be a parallel system. seal-game-editor's 28 `Quest*` types are content-AUTHORING types in the repo that owns authoring; `riir-dapps` defines zero game-rule types, which is the riir-chain Issues 096/097 failure class not recurring. |
