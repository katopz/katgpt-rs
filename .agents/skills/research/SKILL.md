...
product/distillation set is **7** (first block below); the workspace is **24
contract repos** (all with a root `BOUNDARY.md`; this file routes the 7 plus 15
of the 17 non-product repos). Derive the set, never type it:
...
**The wider workspace (the other 15 — route here only when the insight is
inseparable from that surface; the first two entries are first-class PUBLIC
...
1. **`read_file` the product-set READMEs (katgpt-rs, riir-ai, riir-chain, riir-neuron-db, riir-train, riir-game-sdk, riir-dapps) + `riir-ai/.docs/README.md` + `riir-clippy/AGENTS.md` (the healer — fusion priority #2) + `riir-infer/AGENTS.md` + `riir-reflex/AGENTS.md` (the public inference-substrate + decision-serving surfaces) + (for training papers) `riir-train/.docs/02_pipelines/training_data_pipeline.md`** — defines scope boundaries. Skipping = #1 cause of false Super-GOAT claims + false PASS on training papers.
2. **`list_directory` every `.research/` folder in the workspace** — derive it (`ls -d /Users/katopz/git/*/.research`), never trust a typed count: 12 today (katgpt-rs, riir-ai, riir-auth, riir-chain, riir-clippy, riir-dao, riir-neuron-db, riir-reflex, riir-shader, riir-train, riir-viewbridge, seal-online-remaster); create missing ones on first use (riir-infer's is created the first time a distill routes there).
3. **`list_directory` the 6 runtime/chain/db/substrate/serving src trees** — module names are vocab. Skipping = #2 cause of false Super-GOATs.
4. **`web_search` for published prior art** on the paper's headline technique (see §4). Skipping = #4 false-novelty failure mode. **Internal-first rule (GRAM/058 lesson, 2026-09-25): before any web search, grep every workspace `.research/` folder for the paper's arXiv ID AND its title verbatim** — a prior distillation is the controlling constraint, and a content grep that stops at page 1 of matches does not find it (Research 058 was missed exactly this way at note-590 round 1; caught only by the verdict reviewer).
...
- `riir-infer/src/` (the `riir-infer-core` root: `quant/`, `deltanet/`, `transformer/`, `simd/`, `gguf_loader.rs`, `safetensors_loader.rs`, `tokenizer.rs`, ...) + `riir-infer/crates/riir-infer-{gpu,laya}/src/`
- `riir-reflex/src/` (`engine.rs` + `engine/`, `embed.rs`, `label_heads.rs`, `readout.rs`, `harness/`, `lanes/`, `serve.rs`, `game_heads.rs`)
...
3. **Inference-perf league** (beat llama.cpp / oMLX / vLLM — the riir-gpu / riir-ai serving path; substrate-side kernels + quant formats + encoders → `riir-infer`, serving + measurement → `riir-reflex`).
...
| `riir-infer/src/` (the `riir-infer-core` root) + `crates/riir-infer-{gpu,laya}/src/` | quant (`quant/` EXL3-trellis lane, `deltanet/` GDN lane, Bonsai2 rotation), loaders (`gguf_loader.rs`, `safetensors_loader.rs`, `tokenizer.rs`), `transformer/` + `simd/` forwards, the laya encoder lane (flat `Vec<f32>` ops over CPU + Metal MSL + CUDA kernels) | **The substrate half of every serving claim** — maps "quantization / weight format / kernel / encoder port / loader" papers; the G5-parity discipline (bit-identical port vs frozen captures, p-drift ≤ 1e-3) is the template for any forward port |
| `riir-reflex/src/` (`engine.rs` + `engine/`, `embed.rs`, `label_heads.rs`, `readout.rs`) + `harness/` + `lanes/` | decision_wire engine (hashed-feature embedding → `pick_domain` corpus routing → `Lz4FlexDrafter` corpus-is-the-model scoring → sigmoid → `SigmoidGateCalibrator` → fused abstain via `CorpusDistanceGate`) + the dataset harness (9 suites, corpus-cap + cal-select levers, pair-head probes) + comparison lanes (laya CPU/Metal/CUDA, CLM, GLiNER, AgentJev) | **Labeled decision serving with first-class abstention** — maps "calibration / abstention / selective prediction / confidence / comparison arena" papers; published tables are CI-regenerated, never hand-typed, and G5 parity gates every laya number |
...
| `riir-infer` | **Public substrate moat.** Inference-substrate novelty (quant formats, kernels, encoders, loaders) shipped public + upstream-clean (zero riir-* deps) | Weights/quant/architectures/kernels/loaders; EXL3-trellis, GDN, Bonsai2 lanes; the laya encoder lane (CPU/Metal/CUDA); GPU backend layers | Decision serving → `riir-reflex`; training loops/recipes → `riir-train`; engine/game wiring → `riir-ai`; generic modelless math → `katgpt-core` |
| `riir-reflex` | **Decision-serving moat.** Calibrated modelless decisions with first-class abstention + honest comparison arenas (G5 parity gates every published laya number) | `decision_wire` serving, calibration gates, fused abstain, dataset harness + corpus-cap levers, laya/CLM/GLiNER/AgentJev comparison lanes | The laya lane substrate → `riir-infer`; generic primitives → `katgpt-core`; game runtime → `riir-ai`; training → `riir-train` |
