...
// ── SpectralQuant forward path (Plan 077 T8) ────────────────────────
...
///
/// Identical math to [`attention_turboquant`] but accepts flat buffers
/// produced by [`dequantize_spectral_keys_flat`] / [`dequantize_spectral_values_flat`].
#[allow(clippy::too_many_arguments)]
pub fn attention_spectralquant(
    ...
    // Same math as attention_turboquant — KV dequant is already done
...
mod tests {
    ...
    fn test_spectralquant_forward_produces_finite() {
        ...
        use super::super::types::{SpectralQuantCalibration, SpectralQuantKVCacheConfig};
        ...
        let flat_keys = super::dequantize_spectral_keys_flat(&mut sq_cache, 0, 3, kv_dim);
        let flat_values = super::dequantize_spectral_values_flat(&mut sq_cache, 0, 3, kv_dim);
