...
A **GOAT-proved** neuro-symbolic micro-Transformer with speculative decoding, constraint pruning, and **661 feature flags (204 default-on, all GOAT-proved)** — built in Rust. Pure algorithms, zero side effects, MIT licensed.
...
The default production stack has **204 GOAT-proved default-on features** (661 total flags), but they don't all run on every token. The architecture uses **layered gating** — most features are bandit-driven, Option-gated, or compile-time-only.
...
| **KL Effective Depth Probe** (`kl_depth_probe`) | Offline exit calibration for the weight-tied `forward_looped` (Issue 898, Research 592 / arXiv:2609.19107): `LoopDeepRun::capture_logits` logit lens at every loop (shares the Issue-717 tripwire matmul) → `katgpt_core::loop_depth_probe` KL profile vs the final readout, effective depth (first post-peak loop within a KL threshold), write-fraction spectrum + stall onset, loop-flatness spread, executed-depth histogram, half-A fit / half-B holdout threshold calibration. | Opt-in measurement instrument (not a promotion candidate). Bench 899: G1 holdout ±1 loop 1.000/0.969/0.844 ≥ 0.80 on 3 micro fixtures, fit 1.0 nat interior; lens premise bit-exact; capture off/on bit-identical; G4 0 allocs warm. Measured: no Config default change justified (p95 exit is checkpoint-dependent, 2/6/4 of 8). |
| **Trajectory-Aligned Curiosity** (`arm_drift_alignment`) | Per-arm CGSP curiosity gate (Plan 610 / Issue 899, Research 591 / arXiv:2609.30063): `TrajectoryAlignedCuriosity<S: DriftSummary>`. The default `SecondMomentDrift` z-scores the share derivative through the pool's cos² kernel against a simplex-centered null; `FirstMomentDrift` is the Plan 610 comparison arm. | Opt-in — GOAT **FAIL** (Bench 900/901). Second moment: G3 loop wins in both directions (−79 / −63 cycles vs a matched-uniform bonus) and passes the negative control, but misses G1 held-out (0.766) and G4 (2.07×). First moment: amplifies coherent-cluster momentum and loses by 108+ cycles when the better family is spread. |
