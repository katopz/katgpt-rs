...
| **Trajectory-Aligned Curiosity** (`arm_drift_alignment`) | Per-arm CGSP curiosity gate (Plan 610, Research 591 / arXiv:2609.30063): r_k = sigmoid(β·\|⟨ĝ_k, û⟩\|) against the preconditioned drift of the priority-weighted mean pull (`TrajectoryAlignedCuriosity`, `DriftPreconditioner`, `cycle_aligned`). | Opt-in — GOAT **FAIL** (Bench 900): G1 planted-drift AUC 0.783/held-out 0.602 < 0.8; G3 loop A/B wins (230 vs 349 cycles vs matched-uniform) but reverses when the better family has a zero pull centroid (+108 cycles). Mean-pull summary is blind to drift toward spread families; redesign filed as Issue 899. |
