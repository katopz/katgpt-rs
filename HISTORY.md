...
- **Found-by:** the riir-instinct registration (`54edbd097`) — its truthful `0` row red both sweeps at birth, and riir-reflexer had red `len_derived` since its 09-25 registration. A sweep that always reds on a correct repo — the cries-wolf state Issue 793 forbids.
- **Fix (one shared predicate):** `sweep_population.zero_walk_floor_accepted` — the zero delegated row is accepted only while `tracked_walk` (the ONE walk, Issue 777) measures ZERO tracked `.rs` in that repo, re-measured EVERY run; the first `.rs` to land reds exactly as a zeroed row on a code repo does. The acceptance PRINTS (visible on green, never inferred from a missing flag). Both sweeps split their delegation assertion through it; each canary gained an md-only-accepted wiring arm (predicate stubbed — no real repo stays md-only) while the break arms stay on the REAL predicate, so the "code repo with a zeroed row reds" direction is armed live.
- **The measurement caught its own stale premise:** riir-reflexer was registered md-only 2026-09-25, but its vessel workspace had landed **16 tracked `.rs`** by the time the exception shipped — the predicate refused the zero row, and the rows were re-pinned at measured in the same commit (orphaned_attr `16 15 0`; shared_temp_path `5 0`).
- **Carve drift the validation run surfaced (same commit):** the 4 gemv EYES rows re-addressed riir-ai → riir-infer (same binds, new paths; carve `c75cb735ee`/`c261956bea` → `0643575`), +4 fwht rows pinned after reading the provenance (callers bind `TernaryDeltanetGpuForward` struct fields — persistent activation buffers); `len_derived` floors re-pinned at measured: riir-ai `6 10 0` (was `23 78 0` — unreachable since the carve), riir-infer `49 169 0` (was `0 0 0`, typed at carve start before the GPU crates moved).
- **Still red BY DESIGN (riir-infer's owner, not this fix):** one LIVE CAPACITY joined finding at `crates/riir-infer-gpu/src/encoder_lane_cubecl.rs:361 gather_rows_f32 rows_handle [len: rows_host.len(]` — the len-derived bind defect class itself — and 2 fixed temp-path sites (`ane.rs::riir-laya-ane-cache`, `quant/exl3.rs::exl3-pack`) over shared_temp_path's ceiling 0. Both left for the repo's live session; the sweeps stay red until then.
- **Validated:** `sweep_population` selftest 26 assertions; `len_derived` canary 11/13 (both fails = the one riir-infer CAPACITY red above); `shared_temp_path` canary 13/13; real runs report ZERO `DELEGATION broken` flags anywhere and riir-reflexer green in both sweeps. File removed per the noise-reduction rule; git history holds it.
