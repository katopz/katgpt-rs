...
- **Bisected:** live at root from `25aca61b1` (Plan 227 Phase 1); dead since the Issue 015 extraction `b61a34f7b`, which moved it into katgpt-kv without declaring the feature or a `static_cal` module, and added the crate-wide `#![allow(unexpected_cfgs)]` that hid it. So ~3 months of "static cal replaces Sinkhorn in KVarN" claims described code no build compiled.
- **Why not wire — the edge is illegal (measured).** Adding `katgpt-attn = { optional = true }` to katgpt-kv makes `cargo metadata` fail outright with *cyclic package dependency*: katgpt-attn → katgpt-forward → katgpt-pruners → katgpt-speculative → katgpt-kv → katgpt-attn. katgpt-attn also depends on katgpt-kv directly under `dash_attn`, default-on via `vortex_flow`. Cargo rejects the cycle even for an optional edge. BOUNDARY.md has no intra-repo crate rule; the cycle is the decision.
- **Why not re-home it lower instead — the quality gate fails regardless.** The branch divides each tile row by a per-row scale with `s_col = 1`. KVarN's RTN is per-row affine min/max, so any positive per-row scale is absorbed by the RTN range and multiplied back at dequant. The branch is **plain RTN**, not a Sinkhorn replacement. Scratch probe (not committed; 128×128 tiles, the deleted branch transcribed, table scales from `calibrate_from_stats`'s formula):
  - static vs plain RTN: max |Δ| **1.2e-4** (f32 rounding) over 4 families × 3 seeds × {key, val} × bits {2, 3, 4};
  - static quant-MSE ≤ incumbent Sinkhorn in only **6/72** cells (which 6 was not broken out). It loses on every outlier family — the per-family lossy-surface rule fails. Seed-1 figures: val channel-outlier MSE 30.5 / 6.02 / 1.33 vs Sinkhorn 6.78 / 1.26 / 0.267 at 2 / 3 / 4 bits (cosine 0.691 vs 0.899 at 2-bit); key token-outlier 11.2 vs 1.71 at 2-bit (cosine 0.644 vs 0.904); key both-outliers 4-bit 11.7 vs 2.13.
  - latency: the static normalize step costs **0.034×** Sinkhorn (paired `ab_median_ratio`, `--release`, 31 rounds × 40 iters, range 0.028–0.037, `black_box` on both arms). Box: M3, AC, powermode 2, load 13–16, 90% memory free. That is the speed of a no-op.
  - Also wrong in kind: the V-tile branch indexed a per-HEAD table by token position (`rows` = tokens for V), and the K-tile branch by channel.
- **What `static_cal_goat` actually measures:** katgpt-attn standalone — 128 table LOOKUPS vs one Sinkhorn `variance_normalize` of a 128×128 tile. `kvarn` is in its `required-features` only for the baseline import. Its "perplexity delta" is 1 − cosine of two positive scale vectors, so it reads ~0.0000 by construction (re-run: 100.0% / 0.0000, 6/6 pass). It never runs a KVarN quantize path. The root `static_cal_tables` comment, `static_cal.rs` module doc, the test header, and Plan 227 (the "Wire into KVarN" task is retracted, `[-]`) are corrected to say so. The feature stays default-ON as the katgpt-attn table and changes no KVarN numerics.
- **T2:** `#![allow(unexpected_cfgs)]` is removed from `crates/katgpt-kv/src/lib.rs` outright — cargo's built-in check-cfg now warns on any undeclared `feature = ".."` there. It immediately surfaced a **second instance of the same class**: `segment_checkpoint::memory_soup` was gated on `memory_soup_dtree`, never declared in katgpt-kv since the Proposal 003 Phase 5 move (`70d418b8e`), while the root's opt-in `memory_soup_dtree` claimed to gate it. Restored as an opt-in katgpt-kv feature, with the root forwarding it (3 module tests pass; no default change).
- **Checks:** `cargo clippy -p katgpt-kv --all-targets -D warnings` clean at default, `kvarn`, `kvarn,fitted_value_tables`, `kvarn,targeted_precision`, `memory_soup_dtree`, `ssc_spec_draft,memory_soup_dtree`, `mi_probe`, and `--all-features`. Root kvarn consumers (`--lib`, `static_cal_goat`, the 3 kvarn examples, `kv_cache_flatten_bench`, + `memory_soup_dtree`) are clean. katgpt-kv `--lib --test-threads=1`: kvarn 33, kvarn+targeted_precision 37, kvarn+fitted_value_tables 33, memory_soup_dtree+ssc_spec_draft 53, all pass. `count_features`, `cargo_comment_audit` (440 comments, 0 mismatches), `markdown_fence_gate`, and `bench_doc_audit` pass.
...
- **Found while closing, filed as Issue 897 (closed — § Issue 897 above):** katgpt-kv's KVarN `static_cal_tables` branches are gated on a feature katgpt-kv does not declare, and name a `crate::static_cal` module it does not have. `#![allow(unexpected_cfgs)]` hides this, so the path is compiled by nothing.
