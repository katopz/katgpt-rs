...
> **Date:** 2026-09-12
> **Status:** SUPER-GOAT (upgraded 2026-09-12 — the PoC pass condition of §3 was met and the mandatory outputs filed: open primitive `ooo_audit` shipped, GOAT gate Bench 758 G1+G2+G4 ALL PASS, this note's addendum records the measurement, the riir-ai guide landed as Research 378, the riir-clippy axis filed as an issue). Updated 2026-09-12 (b): the F1 consumer pilot LANDED (riir-ai Plan 590 Phase 1, Bench 915 — all four defend-wrong gates PASS; runtime freeze gate owner-gated). Honest scope: what the PoC PROVED is codebank/bank-level redundancy detection + curation on shipped substrate (F3) and the metric-granularity law; the crowd-scale NPC premise (F1) is pilot-proven at POC scale, runtime wiring pending the owner GO. Primitive stays OPT-IN per the no-default-consumer rule.
...
- **F1 consumer pilot LANDED (riir-ai `5f01dafa4`, 2026-09-12):** Plan 590 Phase 1 (Bench 915) — the defend-wrong riir-poc harness on the F1 freeze-time wiring, ALL FOUR GATES PASS (C1 dup-cross 0.562/0.500 < 0.8; C2 kill gate 6/6 kept, cross-bank 0.021; C3 PWC raw 2.43× vs curated 1.00×; C4 0.01–0.43% of the window). Window-model correction for this note's "~90× under a consolidation window": real sleep-cycle cadence is 30–100 ticks × 50 ms = 1.5–5 s (riir-ai `SleepCycleTrigger`), so the U=64/N=1024 headroom is ~1300×, not ~90×; at N=2048 the naive end-of-window sim build alone is ~2% of the tightest window — the runtime wiring should compute sim rows on episode arrival. The runtime freeze gate itself (Phase 2) is owner-gated.
