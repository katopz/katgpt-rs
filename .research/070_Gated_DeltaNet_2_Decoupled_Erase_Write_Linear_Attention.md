...
> **Paper:** [arXiv:2605.22791 — Gated DeltaNet-2: Decoupling Erase and Write in Linear Attention](https://arxiv.org/abs/2605.22791) — Hatamizadeh, Choi, Kautz (NVIDIA), May 2026 (PDF also in the upstream repo: NVlabs/GatedDeltaNet-2 `paper/GDN2_paper.pdf` @ `a5552fe`)
...
>
> **Status (2026-08-25 re-read):** SHIPPED — no further action. E1/E2/E4 live in `katgpt-attn/src/gdn2/`: `gdn2_recurrent_step` + `Gdn2GateConfig::{EraseOnly,Full,Kda}` (the paper's b-only ~90% ablation encoded as the DEFAULT config; `gdn2_attention`), the full KDA layer (Proposal 032 Phase 4 / Research 329; `kda_linear`), the analytic KDA backward (Issue 389 T4, consumed by riir-train Plan 318 C5; `kda_backward`), the tree-verify bridge + tree-forward (Plan 424 T4.2/T4.3; `gdn_tree_verify`), plus the `DiagonalGate` generalization (`diagonal_gate.rs`, shared with Wall). The bridge's documented deferral — exact per-channel decay in tree verify — now has its math reference in this paper §3.3 (asymmetric factors `ē_r = γ_r ⊙ (b_r ⊙ k_r)`, `k̄_r = γ_r⁻¹ ⊙ k_r`); still no consumer (Kimi K3 is KDA-tied-gate, Ternary-Bonsai is scalar-decay GDN). Follow-up paper un-distilled: "Decoupling Erase and Write Addresses in Delta-Rule Linear Attention" (EDA, arXiv:2606.26560) builds directly on this.
>
> **Code delta-distill (2026-09-25):** reference repo read at [NVlabs/GatedDeltaNet-2](https://github.com/NVlabs/GatedDeltaNet-2) @ `a5552fe3c67e0ebc7ef1220df68ae8896ec62d56` — **NVIDIA Source Code License-NC: reference-only, never vendor.** Confirms the SHIPPED status above; deltas, none actionable: (1) **arXiv ID is 2605.22791** — this note previously cited a broken URL (`arxiv.org/pdf/GDN2_paper.pdf`) that was never a working link (grep repair, done above); (2) **reference layer builds on FLA**: `FusedRMSNormSwishGate` output norm, `ShortConvolution` (conv-4), `mamba_init`, optional GVA (`num_v_heads > num_heads`, value-axis head grouping), `allow_neg_eigval` (b×2, arXiv:2411.12537 = the paper's own b∈[0,2] ablation — refuted at 1.3B in §Key Results) — the shipped `kda_forward.rs`/`short_conv.rs` already mirror the parameterization (`gk = -α·softplus(g+dt_bias)`, β = sigmoid, gated output norm); (3) **configs/recipe are reference datapoints**: `layer_idx % gdn2_per_layer == 0` layout → `gdn2_1.3B` pure recurrent, `swa_gdn2_1.3B` **1:1 GDN2:SWA** (9+9 @ n_layer 18, `local_window=2048` SWA), head_dim 128, n_embd 2304, LR 4e-4, FineWeb-Edu 100B, 32 GPUs. The chunkwise WY family (chunk C=64; the gated K–K score matrix `Akk` is the GDN2-specific addition over KDA; backward bakes both gates into the dA accumulation — "a scalar post-scale cannot reconstruct two independent gates living on different axes") restates this note's gate-aware-backward constraint for any FUTURE GDN2 training run; the stack trains scalar-decay GDN (Bonsai) + KDA-tied (K3) today, so no consumer. Open thread recorded in-note, no issue filed (no consumer; precedent = the tree-verify per-channel-decay deferral above; adjudicated at verdict round 1): note 538 never quantizes the post-sigmoid gate at all — what it measured is the SIGMOID SHRINKING a quantized PRE-activation GEMM's error (8.5% GEMM → 5.2% on β → 2.6% layer output, its Table-3(ii)). The open question is whether that shrinking still holds when the `b` projection is `d_k` channels wide per head instead of one scalar per head: a scalar error scales the whole erase uniformly, but per-channel errors land on individual channels and could erase selectively in the WRONG places — a different failure mode that 538's quadrature error-combination rule does not adjudicate. Unmeasured until a GDN2-gated model enters the fleet.
