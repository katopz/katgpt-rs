...
> **PASS-Redirects (synthesis):** Yang et al. [arXiv:2609.02042 "Act More, Decide Less: Skill-Guided Adaptive Action Chunking for Long-Horizon LLM Agents"] — different decision granularity from this gate: per-tick emission gating (here) vs amortizing one decision across a variable-length action chunk whose boundaries are mined from successful trajectories (there); the tri-gate remains the per-tick half of any future trigger ladder — see `.research/532` §9.
