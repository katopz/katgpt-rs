...
> **Decision (2026-09-06, this session):** the registered alternative is CHOSEN — T4.1 kernel DECLINED; first policy at pull time = rand_keystone null. Verified zero attention-KV eviction consumers in riir-ai (router truncation is prompt-side; riir-gpu caches are weight/prefix/segment auxiliary; engine runs fixed-size recurrent state). Pull-gate + wiring contract filed as **riir-ai Issue 882** (`.issues/882_kv_eviction_consumer_pullgate.md`) — the eviction wiring home (Issue 836 is the delta-retention sibling, triggers NOT FIRED). Gate chain stands: `runaway_gate ∧ beats_random_prompt_pin ∧ protection factorial`.
