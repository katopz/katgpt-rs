...
> **PASS-Redirects (synthesis):** Feng et al. [arXiv:2609.28473 "On the Diffusibility of High-Dimensional Latents"] — the RAE-lineage follow-up (ECCV'26, Cornell+Adobe): the RAE diffusibility gap is a parameterization problem (v-pred struggles when reconstruction tuning collapses effective dim, R90 672-to-129 of 1024), solved by x0-prediction at the full 1024-dim latent rather than the adapter-compression route — no plan change here, this note's "we don't train, we don't do diffusion" verdict stands (no image T2I training anywhere in the stack; the discrete analog, D2F clean prediction, is already validated per Research 142 D1). Third-defense grep documented: riir-train's flow-anomaly lane (`flow_anomaly.rs` / `flow_anomaly_tccm.rs`, Plan 350) is the one workspace velocity-field training — examined, does not map: its contraction target −x_t is noiseless (no isotropic ε injection), so the paper's failure mode (capacity spent on the orthogonal ε⊥ component, E‖ε⊥‖² = h−l) cannot arise there, and the tccm module itself documents the −x_t vs −x₀ choice as scorer-convention ("a silent-bias trap, not a preference"), not a parameterization axis.
