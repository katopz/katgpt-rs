...
> **PASS-Redirects (synthesis):** Yang et al. [arXiv:2609.02042 "Act More, Decide Less: Skill-Guided Adaptive Action Chunking for Long-Horizon LLM Agents"] — the decision-scheduling extraction is a seventh vocabulary for this note's covered class (boundary source from mined successful-trajectory task structure is the only new signal; no live consumer — see `.research/532` §3–§4).
> **PASS-Redirects (synthesis):** Jha et al. [arXiv:2609.10817 "Tapes Together Strong: The Co-evolution of Computation and Cooperation"] — the energy-stock axis extends this note's covered class: state-dependent compute budget gated by a metabolic STOCK (not utility flow) with a closed-form starvation law (`2ε < L(1+(1−α)δ)`). Distilled as a NEW primitive (`metabolic_gate`, `.research/550`) rather than covered — the stock axis + design law is the uncovered delta; consumer is riir-ai Plan 585 (thermal-LOD metabolic axis + swarm commons).
