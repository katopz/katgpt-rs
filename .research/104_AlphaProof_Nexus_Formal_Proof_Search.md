...
> **PASS-Redirects (synthesis):** Zenan Li, Kaiyu Yang, Ziran Yang, Zhaoyu Li, Mike He, Aarti Gupta [github.com/Lizn-zn/VeriTile @ 6619023b, MIT — "VeriTile: Formal Verification for Triton Kernels" (Lean 4.29+Mathlib, 2026-09-25, no arxiv yet)] — PASS: agent-driven Lean kernel-verification infra (Claude-Code lean4 plugin + official comparator frozen-snapshot replay) = the Nexus agent-loop-over-checker pattern applied to GPU kernel contracts, plus the trust layer Nexus lacks (statement-surface allowlist, spec-circularity, axiom footprint ⊆ standard base); its permanently-locked two-layer split (Real-arithmetic Lean proof + external empirical IEEE-754 gap checker) independently validates our Lean-spec + empirical-G1 epistemics; nearest unfilled kinship = neuron-db .proofs "Out of scope" row 2 (Merkle completeness level-sizes/EMPTY_HASH padding ↔ their 8 FA-1 boundary variants — same masked/padded index-arithmetic proof shape, no transferable machinery).
