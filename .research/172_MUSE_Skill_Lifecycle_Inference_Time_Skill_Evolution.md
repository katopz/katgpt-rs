...
> **PASS-Redirects (synthesis):** Yang et al. [arXiv:2609.02042 "Act More, Decide Less: Skill-Guided Adaptive Action Chunking for Long-Horizon LLM Agents"] — its two-level skill IR (composite → ordered subskills, each subskill = one chunk) + UCB retrieval + prune-at-zero-success maps onto this note's lifecycle coverage; the new element (subskill boundaries consumed as DECISION-SCHEDULING triggers) has no consumer here — see `.research/532` §3.2 signal-diff.
