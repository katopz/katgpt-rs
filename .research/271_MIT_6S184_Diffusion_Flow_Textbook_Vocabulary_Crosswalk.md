...
| VAE rate-distortion Pareto frontier | "5 scalars across sync boundary" heuristic → **MEASURED (riir-ai Bench 910 / Issue 923 T1–T3, 2026-09-11)**: knee at 5 × u16 = 80 bits (0 bucket-flips/10k, τ=0.99999 vs f32 reference); u8 = 40 bits near-miss (0.63% adjacent-only); **set-reduction refuted** (all 15 subset arms 36–70% flips — width dominates set, the 5-set is informationally minimal; curiosity_drive derives at zero rate). T4 (wire codec change, u16 arm recommended) open, not urgent | measured → riir-ai Issue 923 T4 |
...
- §2 **VAE R-D Pareto row: gap → FILED then MEASURED** — riir-ai `.issues/923_sync_scalar_rate_distortion_frontier.md` (commit `c4f0e453e`, 2026-09-11) and **Bench 910 executed same day** (commit `2f768a855`): knee at 5 × u16 = 80 bits, set-reduction refuted, T4 (wire change) open not-urgent. The "gap → future issue" promise kept and discharged.
