...
- [ ] **T5.1** If G1 passes (phase transition reproduces) AND G3-precursor passes (Jacobian SVD recovers known singular vectors): add `subspace_phase_gate` to the default feature list in `katgpt-rs/Cargo.toml`. **UNBLOCKED** (2026-07-02): G1 passes (Phase 2, re-verified after Issue 008 fix) AND the G3-precursor latency gate (T3.4) PASSES via the zero-alloc `jacobian_svd_at_into` hot path (~800 ns/call < 1µs). T3.1–T3.3 correctness sub-gates all PASS. Ready to promote — deferred to a follow-up commit so this commit stays focused on the T4.1 allocation-elimination fix + its tests/docs.
