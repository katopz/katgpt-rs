...
**Owner call 2026-09-23 (owner-gates menu v2 row 4c): `hint_regret` STAYS opt-in; the two remaining Phase-5 arms (the Setup/Rule/Link modifier-composition variant; the CGSP substrate-side separation term) stay unwired until a consumer needs them.** The landed consumer already migrated onto the primitive (`bae567c`) but runs behind opt-in `demo_coverage_curiosity` — a default flip today would ship an unexercised decision surface in every build (the Bench 681 / `risk_control_exit` posture). Revisit when a default-path consumer lands; if it does, promote+consumer in the same window.
