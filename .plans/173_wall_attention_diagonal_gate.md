...
- [x] Add gate weight `W_g` projection matrix to `TransformerWeights`
...
- [x] Define `WallPrefixState` struct with `prefix_sum: Vec<f32>` (head_dim), updated incrementally each token
- [x] Implement `wall_update_prefix(gate: &[f32], prefix: &mut [f32])` — O(head_dim) per token
- [x] Implement `wall_rescale_query(q: &mut [f32], prefix: &[f32])` — `q *= exp(prefix)`
- [x] Implement `wall_rescale_key(k: &mut [f32], prefix: &[f32])` — `k *= exp(-prefix)`

## Task 4: Forward pass integration

- [x] New `forward_wall` function (or modify existing forward path)
