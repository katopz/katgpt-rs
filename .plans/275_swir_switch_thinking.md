...
**Real-model validation (riir-ai Plan 313) — G2 GATE PASS, 2026-06-18:** Phase 0-4 wiring COMPLETE. **ROOT CAUSE FOUND + FIXED (T4.2d):** The paper default `w_e_to_l=512` was disabling SwiR on Gemma 2 2B (tuned for Qwen3-8B's 800+ token chains; with `max_steps=200` the Explicit→Latent switch never fired). Setting `w_e_to_l=32` enables proper switching: **3.3 L→E switches/problem** (vs 1.0 with broken config). **Results (n=3, w_e_to_l=32, CPU M1 Pro):** G2 token efficiency = **1.32× (GATE PASS, target ≥ 1.3×)**; G1 accuracy = 0% (still blocked by Gemma 2 2B capability — model too small for MATH-500). Latency also improved 20% (14910ms vs 18660ms/problem). **Verdict:** G2 passes — promote `swir_switch_thinking` to default-on once confirmed at n=20+. G1 remains blocked by model capability (not a SwiR design flaw). See [riir-ai `.benchmarks/313_swir_real_model_goat.md`](../../riir-ai/.benchmarks/313_swir_real_model_goat.md).
