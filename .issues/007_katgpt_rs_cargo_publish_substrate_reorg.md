...
- [x] **Phase F (composition-layer unblock):** `katgpt-forward` crate created; `ForwardContext` moved there; root `src/transformer.rs` re-exports it + keeps the 33 `forward*` composition functions. **ACCEPTED 2026-07-02** — Phase F's structural goal (break the `ForwardContext` DAG cycle so substrate leaves are consumable without root) is **fully achieved** by F.1–F.3 + F.4a/F.4b (4 files migrated, GOAT green). The remaining 30 files are pinned by a *deeper* join point (`crate::transformer::forward` the function, not the type) and require an architectural decision (trait-based dispatch vs accept-root-residency) — **tracked in [Issue 033](./033_forward_join_point_architectural_decision.md), NOT blocking Phase F acceptance.** `cargo check --workspace` + `cargo test -p katgpt-core --lib` + root lib tests green after each step.
